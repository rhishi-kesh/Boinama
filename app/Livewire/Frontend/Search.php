...
class Search extends Component
{
    public function render(Request $request)
    {
        $type = $request->book;
        if(!empty($type)) {
            $products = Products::where('name','LIKE','%'.$request->book.'%')->where('status', 0)->where('is_active', 0)->latest()->paginate(20);
        }else{
            ...
            return view('livewire.frontend.main', compact('sliders','miniBanners','latest','subjects','topProducts'))->layout('frontend.app');
        }
        $Subjects = Subjects::get();
        $Prakasanis = Prakasanis::get();
        $Writers = Writers::get();
        return view('livewire.frontend.search', compact('products','Subjects','Prakasanis','Writers'))->layout('frontend.app');
    }
