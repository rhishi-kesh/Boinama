<div class="corporate row" style="background: url('{{ asset('storage/' . $corporates->image) }}')">
    <div class="col-12 col-md-9 col-lg-7">
        <h2>{{ $corporates->title }}</h2>
        <p class="mt-2">{{ $corporates->subtitle }}</p>
        <p class="lead mt-4">{{ $corporates->number }}, {{ $corporates->gmail }}</p>
    </div>
    @section('meta')
